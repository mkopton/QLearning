...
class SarsaAgent:
    ...
    def get_action(self, state):
        ...
        if np.random.rand() < ((1 - self.epsilon) + self.epsilon / nA):
...
class QLearningAgent:
    ...
    def update(self, state, action, reward, next_state):
        ...
        # YOUR CODE HERE
        self.Q_est[state][action] += self.alpha * (reward + self.gamma * self.Q_est[next_state][np.argmax(self.Q_est[next_state])] - self.Q_est[state][action])
